...
**The published four-field example does not prove that the company proxy accepts and forwards `parse_mode` to the Telegram Bot API.** Confirm this with its owner or a safe manual send. If the proxy returns HTTP 400 or 422 for the HTML request, the job retries the same alert as readable plain text with only the documented fields. You can also set `TELEGRAM_ALERT_PARSE_MODE_FIELD=` while waiting for proxy support. A proxy that silently ignores `parse_mode` may still show literal HTML markup. Without forwarding the parse mode, Telegram cannot be guaranteed to render bold or code blocks as rich text. Do not claim this integration has been end-to-end tested before a live send.
