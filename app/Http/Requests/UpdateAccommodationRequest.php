...
class UpdateAccommodationRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'accommodation_type_id' => 'sometimes|nullable|integer|exists:accommodation_types,id',
            'star' => 'sometimes|nullable',
        ...
        foreach ($rules as $field => &$pipe) {
            ...
            foreach ($parts as &$p) {
                if (strpos($p, 'unique:') === 0) {
                    $p2 = substr($p, 7);
                    $tmp = explode(',', $p2, 2);
                    $tbl = $tmp[0] !== '' ? $tmp[0] : 'accommodations';
                    $col = isset($tmp[1]) && $tmp[1] !== '' ? $tmp[1] : $field;

                    $id = null;
                    $routeParam = $this->route('accommodation');
                    if ($routeParam) {
