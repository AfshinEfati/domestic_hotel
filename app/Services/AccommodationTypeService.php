<?php

namespace App\Services;

use App\Services\Contracts\AccommodationTypeServiceInterface;
use App\Services\BaseService;
use App\Models\AccommodationType;
use App\Repositories\Contracts\AccommodationTypeRepositoryInterface;
use App\DTOs\AccommodationTypeDTO;

class AccommodationTypeService extends BaseService implements AccommodationTypeServiceInterface
{
    public function __construct(AccommodationTypeRepositoryInterface $repository)
    {
        parent::__construct($repository);
    }

    /**
     * @return iterable<AccommodationType>
     */
    public function index(): iterable
    {
        /** @var iterable<AccommodationType> */
        return parent::index();
    }

    public function show(int|string $id): ?AccommodationType
    {
        /** @var AccommodationType|null */
        return parent::show($id);
    }

    /**
     * @param AccommodationTypeDTO|array $payload
     * @return AccommodationType
     */
    public function store(mixed $payload): AccommodationType
    {
        if ($payload instanceof AccommodationTypeDTO) {
            $payload = $payload->toArray();
        }

        /** @var AccommodationType */
        return parent::store($payload);
    }

    /**
     * @param int|string $id
     * @param AccommodationTypeDTO|array $payload
     */
    public function update(int|string $id, mixed $payload): bool
    {
        if ($payload instanceof AccommodationTypeDTO) {
            $payload = $payload->toArray();
        }

        return parent::update($id, $payload);
    }

    public function destroy(int|string $id): bool
    {
        return parent::destroy($id);
    }
}
