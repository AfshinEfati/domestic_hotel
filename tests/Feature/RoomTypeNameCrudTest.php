...
class RoomTypeNameCrudTest extends TestCase
{
    use RefreshDatabase, WithFaker;

    protected string $baseUri = '/test-room-type-names';
    ...
    protected function setUp(): void
    {
        ...
        Route::middleware('api')->group(function () {
            Route::apiResource('test-room-type-names', \App\Http\Controllers\Api\V1\RoomTypeNameController::class);
        });
    }

    private function fillable(): array
    {
        return ['fa_name', 'en_name'];
    }
    ...
    private function buildValidPayload(bool $forCreate = true): array
    {
        ...
        if (method_exists(\App\Models\RoomTypeName::class, 'factory')) {
            try {
                $model = \App\Models\RoomTypeName::factory()->make();
                $data = $model->toArray();
    ...
    private function createModel(): \App\Models\RoomTypeName
    {
        if (method_exists(\App\Models\RoomTypeName::class, 'factory')) {
            return \App\Models\RoomTypeName::factory()->create();
        }

        [$payload] = $this->buildValidPayload(true);

        return \App\Models\RoomTypeName::query()->create($payload);
    }
    ...
    public function test_index_returns_list(): void
    {
        try {
            if (method_exists(\App\Models\RoomTypeName::class, 'factory')) {
                \App\Models\RoomTypeName::factory()->count(3)->create();
            }
        } catch (\Throwable $e) {}
